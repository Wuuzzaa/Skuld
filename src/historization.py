...
def _create_history_tables_and_view_if_not_exist(source_table: str):
    ...
    history_time_travel_view_name = f"{table}HistoryTimeTravel"
    if not table_function_exists(history_time_travel_view_name) or True:
        path_sql_create_view_statements = pathlib.Path("db/SQL/views/create_view/history/table_time_travel_views")
        path_sql_create_view_statements.mkdir(parents=True, exist_ok=True)
        create_view_sql = get_history_time_travel_view_create_statement(table)
        with open(f"{path_sql_create_view_statements}/{history_time_travel_view_name}.sql", "w") as f:
            f.write(create_view_sql)
        with pg_engine.begin() as connection:
            execute_sql(connection, f'DROP VIEW IF EXISTS "{history_time_travel_view_name}" CASCADE;', history_time_travel_view_name, "DROP VIEW")
            execute_sql(connection, create_view_sql, history_time_travel_view_name, "CREATE VIEW")
...
def get_history_time_travel_view_create_statement(table_name: str):
    """
    Create statement for history table function of the specified table.
    """
    history_select = get_history_select_statement(table_name, optimized=True, time_travel=True)

    create_statement_sql =  f"""
    -- generated by HistorizationService
    CREATE VIEW
        "{table_name}HistoryTimeTravel" AS
    {history_select}
    """
    return create_statement_sql
    ...
    if str(date) != str(time.strftime("%Y-%m-%d", time.gmtime())):
        ...
        if mode == 'TimeTravelViews':
            for view in HISTORY_ENABLED_VIEWS:
                ...
                sql = sql.replace(f'"{view}"', f'"{history_time_travel_view}"')
            for table in HISTORY_ENABLED_TABLES:
...
def generate_time_travel_views_for_history_enabled_views():
    # take all views in HISTORY_ENABLED_VIEWS and generate a table function for each view which takes a date as input and returns the data for that date from the history view, if the date is not the current date, otherwise it returns the data from the current view without filtering on date
    # wrap select in function, replace views with history table function and pass down the date parameter
#    get select for views using 
#  SELECT definition 
...
#   repplace views and tables with "get{view}History(p_target_date)" and pass down the date parameter to the function
    for view in HISTORY_ENABLED_VIEWS:
        history_view = f"{view}History"
        time_travel_view_name = f"{view}HistoryTimeTravel"
        # get select statement for the view using pg_views
        view_select = select_into_dataframe(f"""
        SELECT definition
        FROM pg_views 
        ...
        # replace view name with history table function and pass down the date parameter
        for view_tbr in HISTORY_ENABLED_VIEWS:
            view_select = view_select.replace(f'"{view_tbr}"', f'"{view_tbr}HistoryTimeTravel"')
        
        for table_tbr in HISTORY_ENABLED_TABLES:
            view_select = view_select.replace(f'"{table_tbr}"', f'"{table_tbr}HistoryTimeTravel"')
        
        # replace occurences of CURRENT_DATE with the provided date
        view_select = view_select.replace("CURRENT_DATE", f"current_setting('app.time_travel_date', true)::date")

        # generate function sql
        time_travel_view_sql = f"""
        -- generated by HistorizationService
        CREATE OR REPLACE VIEW
             "{time_travel_view_name}" AS
        {view_select};
                """

        # write sql to file "get{view}History.sql" to /Users/jakobingenfeld/Development/Options/Skuld/db/SQL/views/PostgreSQL/history/time_travel_views/
        path_sql_create_view_statements = pathlib.Path("db/SQL/views/PostgreSQL/history/time_travel_views")
        path_sql_create_view_statements.mkdir(parents=True, exist_ok=True)
        with open(f"{path_sql_create_view_statements}/{time_travel_view_name}.sql", "w") as f:
            f.write(time_travel_view_sql)
        ...
        if pg_engine:
            with pg_engine.begin() as connection:
                try:
                    execute_sql(connection, f'DROP VIEW IF EXISTS "{time_travel_view_name}" CASCADE;', time_travel_view_name, "DROP VIEW")
                    execute_sql(connection, time_travel_view_sql, time_travel_view_name, "CREATE VIEW", f"Create time travel view {time_travel_view_name}")
                except Exception as e:
                    logger.error(f"Error creating time travel view {time_travel_view_name}: {e}")
                    raise e