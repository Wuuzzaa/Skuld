...
def upload_database_to_drive(service_account_file, file_path, file_name, parent_folder_id):
    """
    Uploads the SQLite database file to Google Drive using a Service Account.
    Checks if a file with the same name already exists in the target folder. If so, it deletes that file.

    :param service_account_file: Path to the Service Account JSON file
    :param file_path: Local path to the database file to be uploaded
    :param file_name: Name of the file on Google Drive
    :param parent_folder_id: ID of the target folder on Google Drive
    :return: ID of the uploaded file or None in case of an error
    """
    try:
        ...
        # Upload the database file with application/x-sqlite3 mimetype
        media = MediaFileUpload(file_path, mimetype="application/x-sqlite3", chunksize=256*1024, resumable=True)
        request = service.files().create(
            body=file_metadata,
            media_body=media,
            fields="id"
...
def upload_database():
    """
    Executes the upload of the SQLite database to Google Drive.
    This function encapsulates the upload step and internally calls upload_database_to_drive.
    """
    service_account_file = PATH_FOR_SERVICE_ACCOUNT_FILE  # Path to the Service Account JSON file
    parent_folder_id = PATH_ON_GOOGLE_DRIVE  # Target folder ID in Google Drive
