...
def _calculate_combined_metrics(row: pd.Series, iv_correction: str = 'auto', risk_free_rate: float = RISK_FREE_RATE) -> pd.Series:
...
def _calculate_iron_condor_metrics(df: pd.DataFrame, iv_correction: str = 'auto', risk_free_rate: float = RISK_FREE_RATE) -> pd.DataFrame:
    ...
    metrics_df = df.apply(lambda r: _calculate_combined_metrics(r, iv_correction=iv_correction, risk_free_rate=risk_free_rate), axis=1)
