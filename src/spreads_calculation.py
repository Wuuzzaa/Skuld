...
def _calculate_metrics_for_row(row: pd.Series, strategy_type: str = 'credit', iv_correction: str = 'auto', 
    ...
    legs = [
        OptionLeg(
            ...
            delta=float(row.get('sell_delta', 0.0)) * (1 if row['option_type'] == 'call' else -1),
            gamma=0.0,
            vega=0.0,
            theta=float(row.get('sell_theta', 0.0)),
            iv=float(row.get('sell_iv', 0.0)),
        ...
        OptionLeg(
            ...
            is_call=row['option_type'] == 'call',
            is_long=is_credit,
            delta=float(row.get('buy_delta', 0.0)) * (1 if row['option_type'] == 'call' else -1),
            gamma=0.0,
            vega=0.0,
            theta=float(row.get('buy_theta', 0.0)),
            iv=float(row.get('buy_iv', 0.0)),
