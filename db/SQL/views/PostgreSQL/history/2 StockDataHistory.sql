DROP VIEW IF EXISTS "StockDataHistory" CASCADE;
...
SELECT
	...
	CAST(a.date - g.from_date AS INTEGER) AS days_of_options_data_history,
	...
	CAST(a.date - h.from_date AS INTEGER) AS days_of_stock_prices_history,
...
FROM
	...
	LEFT OUTER JOIN "DividendDataHistory" AS F ON a.date = f.date AND A.SYMBOL = F.SYMBOL
	LEFT OUTER JOIN (
	...
	LEFT OUTER JOIN "FundamentalDataHistory" AS I
	ON a.date = i.date AND A.SYMBOL = I.SYMBOL
	LEFT OUTER JOIN "TechnicalIndicatorsCalculatedHistory" AS J
	ON a.date = j.date AND A.SYMBOL = J.SYMBOL
	-- LEFT OUTER JOIN "TableLastUpdatedHistory" AS tlu
	-- ON a.date = tlu.date AND tlu.table_name = 'StockPricesYahoo';