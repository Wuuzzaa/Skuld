...
SELECT
	...
	ROUND(
		ROUND(c.analyst_mean_target - d.close, 2) / d.close * 100.0,
		2
	) as "target-close%",
	ROUND(
		d.close * a.iv * sqrt(a.days_to_expiration / 365.0),
		2
	) as expected_move
FROM
	OptionDataHistory AS a
	JOIN FundamentalDataHistory as b ON a.symbol = b.symbol
	AND a.date = b.date
	LEFT OUTER JOIN StockDataHistory as c ON a.symbol = c.symbol
	AND a.date = c.date
	LEFT OUTER JOIN TechnicalIndicatorsHistory as d ON a.symbol = d.symbol
	AND a.date = d.date;