...
from src.iron_condor_calculation import calc_strangles, get_page_strangles
...
DEFAULTS = {
    'ss_show_monthly': DEFAULT_SHOW_MONTHLY,
    'ss_show_weekly': DEFAULT_SHOW_WEEKLY,
    'ss_show_daily': DEFAULT_SHOW_DAILY,
    'ss_delta_put': DEFAULT_DELTA_TARGET,
    'ss_delta_call': DEFAULT_DELTA_TARGET,
    'ss_min_sell_iv': DEFAULT_MIN_SELL_IV,
    'ss_max_sell_iv': DEFAULT_MAX_SELL_IV,
    'ss_min_credit': DEFAULT_MIN_CREDIT,
    'ss_delta_candidates': DEFAULT_DELTA_CANDIDATES,
    'ss_asset_type': 'All',
    'ss_sectors': [],
    'ss_show_only_no_earnings': DEFAULT_SHOW_ONLY_NO_EARNINGS,
}
...
_strat = st.session_state.get('ss_strategy_label', 'Short Strangle')
st.title("Short Strangle")
st.caption("Sell OTM Put + Sell OTM Call — undefined risk both sides, high IV environment.")

init_session_state(DEFAULTS)


def _on_asset_type_change():
    if st.session_state.ss_asset_type in ('Indices', 'ETFs'):
        st.session_state.ss_min_sell_iv = 0.0
        st.session_state.ss_delta_candidates = 5
        st.session_state.ss_show_only_no_earnings = False
...
def clear_all_filters():
    st.session_state.ss_show_monthly = True
    st.session_state.ss_show_weekly = True
    st.session_state.ss_show_daily = True
    st.session_state.ss_min_sell_iv = 0.0
    st.session_state.ss_max_sell_iv = 999.0
    st.session_state.ss_min_credit = 0.0
    st.session_state.ss_show_only_no_earnings = False
...
with st.expander("Configuration and Filters", expanded=True):
    ...
    with asset_col:
        st.radio("Asset Type", ['All', 'Stocks', 'ETFs', 'Indices'],
                 key='ss_asset_type', horizontal=True, on_change=_on_asset_type_change)
    with sector_col:
        ...
        st.multiselect("Sectors (empty = all)", sector_list, key='ss_sectors')
    ...
    with col1:
        ...
        filtered_dates = filter_by_expiration_type(
            ...
            st.session_state.ss_show_monthly,
            st.session_state.ss_show_weekly,
            st.session_state.ss_show_daily)
    ...
    with col2:
        st.number_input("Put Delta Target", 0.0, 1.0, step=0.01, key="ss_delta_put")
        st.number_input("Call Delta Target", 0.0, 1.0, step=0.01, key="ss_delta_call")
        st.number_input("Delta Candidates", 1, 10, step=1, key="ss_delta_candidates",
                        help="delta_rank ≤ N — mehr Kandidaten = mehr Symbole")

    with col3:
        st.number_input("Min Total Credit ($)", 0.0, step=10.0, key="ss_min_credit")
        st.number_input("Min Sell IV", 0.0, step=0.05, format="%.2f", key="ss_min_sell_iv")
        st.number_input("Max Sell IV", 0.0, step=0.05, format="%.2f", key="ss_max_sell_iv")

    col4, col5, col6 = st.columns(3)
    with col4:
        st.checkbox("Show Monthly", key="ss_show_monthly")
    with col5:
        st.checkbox("Show Weekly", key="ss_show_weekly")
    with col6:
        st.checkbox("Show Daily", key="ss_show_daily")
    ...
    st.checkbox("Show only symbols with no earnings till expiration", key="ss_show_only_no_earnings")
...
@st.cache_data
def _cached_calc(put_df, call_df):
    return calc_strangles(put_df, call_df)


@st.cache_data
def _cached_page(df):
    return get_page_strangles(df)


with st.spinner("Calculating Short Strangles..."):
    common_params = {
        "min_open_interest": min_open_interest,
        "min_day_volume": min_day_volume,
        "min_iv_rank": min_iv_rank,
        "min_iv_percentile": 0,
        "delta_candidates": st.session_state.ss_delta_candidates,
    }
    sql_path = PATH_DATABASE_QUERY_FOLDER / 'short_strangle_input.sql'
    put_params  = {**common_params, "expiration_date": expiration_date, "option_type": "put",  "delta_target": st.session_state.ss_delta_put}
    call_params = {**common_params, "expiration_date": expiration_date, "option_type": "call", "delta_target": st.session_state.ss_delta_call}

    with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
        f_put  = executor.submit(_cached_select, date=selected_date, sql_file_path=sql_path, params=put_params)
        f_call = executor.submit(_cached_select, date=selected_date, sql_file_path=sql_path, params=call_params)
        put_df  = f_put.result()
        call_df = f_call.result()

    raw = _cached_calc(put_df, call_df)
    df  = _cached_page(raw)
...
if not df.empty:
    ...
    asset_filter = st.session_state.ss_asset_type
    ...
    if st.session_state.ss_sectors:
        df = df[df['company_sector'].isin(st.session_state.ss_sectors)]

    # Best candidate per symbol
    df['_delta_dist'] = (df['delta_put'] - st.session_state.ss_delta_put).abs() + \
                        (df['delta_call'] - st.session_state.ss_delta_call).abs()
    df = df.sort_values('_delta_dist').drop_duplicates(subset=['symbol'], keep='first')
    df = df.drop(columns=['_delta_dist'])

if not df.empty:
    df = df[df['total_credit_dollar'] >= st.session_state.ss_min_credit]

    if 'sell_iv' in df.columns:
        df = df[df['sell_iv'] >= st.session_state.ss_min_sell_iv]
        df = df[df['sell_iv'] <= st.session_state.ss_max_sell_iv]

    if st.session_state.ss_show_only_no_earnings:
...
if not df.empty:
    ...
    st.info(
        "⚠️ Short Strangle = **undefined risk** beidseitig. "
        "Breakeven Put = Strike Put − Credit · Breakeven Call = Strike Call + Credit."
    )
    column_config = {
