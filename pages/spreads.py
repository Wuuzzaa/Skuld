    ...
    if selected_rows:
        ...
        legs = [
            OptionLeg(
                ...
                delta=float(row.get('sell_delta', 0.0)), gamma=0.0, vega=0.0,
                theta=float(row.get('sell_theta', 0.0)), iv=float(row.get('sell_iv', 0.0)),
            ...
            OptionLeg(
                ...
                delta=float(row.get('buy_delta', 0.0)), gamma=0.0, vega=0.0,
                theta=float(row.get('buy_theta', 0.0)), iv=float(row.get('buy_iv', 0.0)),
        ...
        display_strategy_details(row['symbol'], row.get('Company', 'N/A'), legs, metrics, extra_info, 
                                 context={'underlying_price': row.get('close', 100.0), 
                                          ...
                                          'dte': int((pd.to_datetime(expiration_date) - today).days),
                                          'take_profit': st.session_state.take_profit,
                                          'stop_loss': st.session_state.stop_loss,
                                          'dte_close': st.session_state.dte_close})
