"""
Position Insurance Tool – Streamlit page.
...
PowerOptions-style UI for the RadioActive Trading method:
  ...
  * Click-to-explain inline documentation
"""
...
# ── Page Title ──────────────────────────────────────────────────────
st.title("🛡️ Position Insurance Tool")
st.caption(
...
with c_sym:
    symbol_input = st.text_input(
        "Stock Symbol",
        value=st.session_state.get("pit_symbol", ""),
        key="pit_sym_input",
    ).upper()
with c_cb:
    ...
    with st.spinner(f"Lade Optionen für {symbol_input}..."):
        try:
            ...
            if all_df.empty:
                ...
                st.session_state["pit_puts_df"] = None
                st.session_state["pit_calls_df"] = None
            else:
                ...
                st.session_state["pit_puts_df"] = puts_df if not puts_df.empty else None
                st.session_state["pit_calls_df"] = calls_df if not calls_df.empty else None
                st.session_state["pit_current_price"] = float(current_price)
                st.session_state["pit_symbol"] = symbol_input
                st.rerun()
...
# DISPLAY
# =====================================================================
if st.session_state["pit_puts_df"] is not None:
    puts_df = st.session_state["pit_puts_df"].copy()
    calls_df = st.session_state["pit_calls_df"]
    current_price = st.session_state["pit_current_price"]
    symbol = st.session_state["pit_symbol"]
    ...
    if filtered_puts.empty:
        ...
        if collar_enabled and "call_label" in result_df.columns:
            ...
            header = f"Position Insurance – {sel_put_month_label}"
...
# ── Footer ──────────────────────────────────────────────────────────
st.divider()
st.caption("Position Insurance Tool – RadioActive Trading Absicherungsstrategie")
