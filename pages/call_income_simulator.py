...
# STEP 1 – Position & Put
# =====================================================================
st.subheader("Schritt 1: Position & Put")
...
with c2:
    cis_put_strike = st.number_input("Put Strike", min_value=0.01, value=120.0, step=1.0, format="%.2f", key="cis_ps")
    cis_put_price = st.number_input("Put Preis (bezahlt)", min_value=0.01, value=8.0, step=0.5, format="%.2f", key="cis_pp")

cis_put_exp = st.date_input(
    "Put Verfallsdatum",
...
# Load call data
load_btn = st.button("📥 Call-Daten laden", type="primary", use_container_width=True)

if load_btn and cis_symbol:
    with st.spinner(f"Lade Call-Daten für {cis_symbol}..."):
        try:
            params = {
                "symbol": cis_symbol,
            ...
            if all_df.empty:
                ...
                if calls_df.empty:
                    st.warning("Keine Call-Optionen verfügbar.")
                    st.session_state["cis_calls_df"] = None
                else:
                    # Filter: only OTM/ATM calls
                    calls_df = calls_df[calls_df["strike_price"] >= current_price].copy()
                    # Filter: only expirations before put expiry
...
# Show derived info if data is loaded
if st.session_state["cis_calls_df"] is not None:
    ...
    st.success(
        f"**{st.session_state['cis_symbol']}** – Aktueller Kurs: **{cp:.2f}$** &ensp;|&ensp; "
        f"New Cost Basis (mit Put): **{ncb:.2f}$** &ensp;|&ensp; "
        f"Locked-in Profit: **{lip:.2f}$ ({lip_pct:.1f}%)** &ensp;|&ensp; "
...
# =====================================================================
if st.session_state.get("cis_step", 1) >= 2 and st.session_state["cis_calls_df"] is not None:
    st.divider()
    st.subheader("Schritt 2: Call-Strategie")

    calls_df = st.session_state["cis_calls_df"]
